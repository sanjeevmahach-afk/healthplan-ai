  ...
  return (
    <div>
      ...
      <div style={{ position:"relative", marginBottom:"8px" }}>
        ...
        <div style={{ position:"relative", height:"6px", marginBottom:"3px" }}>
          {SLABS.map((s, i) => {
            const pct = Math.min(97, (s.min / THAILAND_TOTAL) * 100);
            const bAch = booked >= s.min;
            const sAch = sourced >= s.min;
            const color = bAch ? C.green : sAch ? C.accent : "#ccc";
            return (
              <div key={i} style={{ position:"absolute", left: pct + "%", transform:"translateX(-50%)", width:"1.5px", height:"6px", background:color }} />
            );
          })}
        </div>

        {/* Bar */}
        <div style={{ height:"12px", background:C.border, borderRadius:"99px", overflow:"hidden", position:"relative" }}>
        ...
        <div style={{ position:"relative", height:"18px", marginTop:"3px" }}>
          {SLABS.map((s, i) => {
            ...
            return (
              <div key={i} style={{ position:"absolute", left: pct + "%", transform:"translateX(-50%)", fontSize:"9px", fontWeight:700, color, whiteSpace:"nowrap" }}>
                {s.amt}
              </div>
            );
          })}
      ...
      {/* Reward cards - shown as a scrollable row */}
      <div style={{ display:"flex", gap:"6px", overflowX:"auto", paddingBottom:"4px", marginBottom:"6px" }}>
        {SLABS.map((s, i) => {
          const bAch = booked >= s.min;
          const sAch = sourced >= s.min;
          const bg    = bAch ? "#F0FDF4" : sAch ? "#FFF8E1" : C.bg;
          const border = bAch ? "#86efac" : sAch ? "#FCD34D" : C.border;
          const color  = bAch ? C.green   : sAch ? "#B45309" : C.muted;
          return (
            <div key={i} style={{ flexShrink:0, background:bg, border:`1px solid ${border}`, borderRadius:"8px", padding:"6px 8px", textAlign:"center", minWidth:"72px" }}>
              <div style={{ fontSize:"11px", fontWeight:700, color, marginBottom:"2px" }}>{s.amt}</div>
              <div style={{ fontSize:"10px", color, opacity:0.8 }}>{s.reward}</div>
            </div>
          );
        })}
      </div>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:"8px", marginBottom:"22px" }}>
                    ...
                    <div style={{ background:C.bg, border:`1.5px solid ${C.border}`, borderRadius:C.radiusSm, padding:"10px", textAlign:"center" }}>
                      ...
                      <div style={{ fontSize:"18px", fontWeight:700, color:C.accentText }}>
                        {vliAmount > 0 ? "₹" + Math.round(parseFloat(vliAmount)).toLocaleString("en-IN") : "₹0"}
                      </div>
                    </div>
                  </div>
